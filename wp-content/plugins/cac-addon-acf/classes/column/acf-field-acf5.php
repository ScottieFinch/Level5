...
class CPAC_ACF_Column_ACF_Field_ACF5 extends CPAC_ACF_Column_ACF_Field {
	...
	public function get_value( $id ) {
		...
		if ( $do_format ) {
			$rawvalue = get_field( $this->get_field_hash(), $this->get_formatted_id( $id ), true );
		}
		else {
						...
						<?php foreach ( $optiongroups as $group ) : ?>
							<optgroup label="<?php echo esc_attr( $group['title'] ); ?>">
								<?php foreach ( $group['options'] as $field_key => $option ) : ?>
									<option data-field-type="<?php echo esc_attr( $option['type'] ); ?>" value="<?php echo $field_key ?>"<?php selected( $field_key, $this->get_field_hash() ) ?>><?php echo $option['label'] ? $option['label'] : __( 'empty label', 'codepress-admin-columns' ); ?></option>
								<?php endforeach; ?>
							</optgroup>
						<?php endforeach; ?>
					</select>
